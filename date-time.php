...
<body>
  ...
  <p>
    <?php
    // 私用するデフォルトのタイムゾーンを指定する
    date_default_timezone_set('Asia/Tokyo');

    // 現在の日時を指定したフォーマットで出力する
  echo date('Y月n月j日H時i分s秒') . '<br>';
  ...
  <p>
    ...
    echo $date_time->format('Y年n月j日H字i分s秒') . '<br>';

    // 現在の日時のインスタンスを作成する
    $now = new DateTime();

    // diff()メソッドで2つのインスタンスの日時の差を取得し、変数$intervalに代入する（interval＝感覚）
    $interval = $now->diff($date_time);

    // 取得した日時の差を特定のフォーマットで出力する
    echo $interval->format('%y年と%m月と%d日の差があります。総日数は%a日です。') . '<br>';
    
    // 加算・減算してもインスタンス$nowの日時が変わらないようにDateTimeImmutableクラスを使って作成する  
